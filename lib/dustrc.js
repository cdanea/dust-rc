...
var DustModule = function (config) {
  var
    _ = require('underscore'), fs = require('fs'), path = require('path'), dust = require('dustjs-linkedin'),
    options = _.extend({}, require('./config'), config),
    extension = options.ext, source = options.src, dest = options.out,
    dustrc = {
      options: options,
      parse: function (base, ext, callback) {
        ...
        fs.readdir(base, function (err, files) {
          _.filter(files, function (file) {
            ...
            /*filtered*/.forEach(
            function (fileName) {
              var realPath = path.join(base, fileName);
